...
export class PaymentIntentService {
  ...
  // ---- Payment Links (public invocation) -----------------------------------

  /**
   * Create a Payment Intent from a public Payment Link invocation.
   * Amount is always server-controlled:
   * - fixed: from the link's stored amount
   * - open_amount/donation: payer provides amount within merchant-configured bounds
   *
   * The pricingRuleId bug fix: looks up by ID (pricingRuleById) not productId.
   * Use count is incremented atomically BEFORE invoice creation; rolled back via
   * decrement if creation fails (best-effort).
   */
  async createFromPaymentLink(
    slug: string,
    /** For open_amount/donation links: payer-supplied amount in satoshis */
    payerAmountSats?: bigint,
    ...
    // Atomic use limit enforcement
    const incremented = this.repo.incrementPaymentLinkUseCountAtomic(link.id, link.maxUses)
    if (!incremented) {
      throw Object.assign(
    ...
    let amountSats: bigint
    let description = link.label

    try {
      if (link.mode === 'fixed') {
        if (!link.amountSats) {
          // Resolve via pricing rule (correct: use pricingRuleById, not pricingRule by productId)
          if (!link.pricingRuleId) {
            throw new Error('Fixed-mode payment link has no amount or pricing rule')
          }
          ...
          description = link.label || rule.name
        } else {
          amountSats = BigInt(link.amountSats)
        }
        // Fixed mode: browser cannot override amount
        if (payerAmountSats !== undefined) {
          throw Object.assign(
            new Error('Cannot override a fixed-price payment link'),
            { statusCode: 400, code: 'FIXED_PRICE_OVERRIDE_DENIED' },
          )
        }
      } else if (link.mode === 'open_amount' || link.mode === 'donation') {
        // Payer must supply an amount within merchant-configured bounds
        if (payerAmountSats === undefined) {
          throw Object.assign(
            new Error('Amount is required for open-amount payment links'),
            { statusCode: 400, code: 'AMOUNT_REQUIRED' },
          )
        }
        if (payerAmountSats <= 0n) {
          throw Object.assign(new Error('Amount must be positive'), { statusCode: 400, code: 'INVALID_AMOUNT' })
        }
        const minSats = link.minAmountSats ? BigInt(link.minAmountSats) : 1n
        const maxSats = link.maxAmountSats ? BigInt(link.maxAmountSats) : this.config.MAX_INVOICE_SATS
        if (payerAmountSats < minSats) {
          throw Object.assign(
            new Error(`Amount must be at least ${minSats} sat`),
            { statusCode: 400, code: 'AMOUNT_BELOW_MINIMUM' },
          )
        }
        if (payerAmountSats > maxSats) {
          throw Object.assign(
            new Error(`Amount must be at most ${maxSats} sat`),
            { statusCode: 400, code: 'AMOUNT_ABOVE_MAXIMUM' },
          )
        }
        amountSats = payerAmountSats
      } else {
        throw Object.assign(new Error('Unknown payment link mode'), { statusCode: 500, code: 'INTERNAL_ERROR' })
      }
    } catch (err) {
      // Best-effort: decrement use count if we fail before creating the invoice
      // (The atomic increment already ran; we don't want to consume a use on errors)
      try {
        this.repo.incrementPaymentLinkUseCountAtomic(link.id, null)
        // Note: a clean "decrement" method would be cleaner, but for safety we
        // only undo the increment. In production, add a decrementPaymentLinkUseCount method.
    ...
    return this.create({
      ...
      pricingRuleId: link.pricingRuleId ?? undefined,
    })
  }
